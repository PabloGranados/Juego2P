...
class FileManager(private val context: Context) {
    ...
    private fun parseTextBoardSimple(boardLines: List<String>): List<List<com.example.buscaminas.model.Cell>> {
        ...
        return if (board.size == 10) board else createEmptyBoard()
            ...
            val player1Wins = if (player1Index >= 0 && player1Index + 2 < lines.size) {
                lines[player1Index + 2].substringAfter("Victorias:").trim().toIntOrNull() ?: 0
            } else 0
            
            // Parsear jugador 2
            val player2Index = lines.indexOfFirst { it.startsWith("Jugador 2:") }
            val player2Name = if (player2Index >= 0) {
                lines[player2Index].substringAfter("Jugador 2:").trim()
            } else "Jugador 2"
            
            val player2Points = if (player2Index >= 0 && player2Index + 1 < lines.size) {
                lines[player2Index + 1].substringAfter("Puntos:").trim().toIntOrNull() ?: 0
            } else 0
            
            val player2Wins = if (player2Index >= 0 && player2Index + 2 < lines.size) {
                lines[player2Index + 2].substringAfter("Victorias:").trim().toIntOrNull() ?: 0
            } else 0
            ...
            val boardStartIndex = lines.indexOfFirst { it.startsWith("Tablero:") }
            val board = if (boardStartIndex >= 0 && boardStartIndex + 1 < lines.size) {
                parseTextBoard(lines.subList(boardStartIndex + 1, lines.size))
            } else {
                createEmptyBoard()
            }
            
            // Parsear información del juego
            val currentPlayer = lines.find { it.startsWith("Turno:") }
                ?.substringAfter("Turno:")?.trim()?.toIntOrNull() ?: 1
            ...
            val placedFlags = lines.find { it.startsWith("Banderas colocadas:") }
                ?.substringAfter("Banderas colocadas:")?.trim()?.toIntOrNull() ?: 0
            
            val totalFlags = lines.find { it.startsWith("Banderas totales:") }
                ?.substringAfter("Banderas totales:")?.trim()?.toIntOrNull() ?: 0
            
            val isFirstMove = lines.find { it.startsWith("Primer movimiento:") }
                ?.substringAfter("Primer movimiento:")?.trim()?.toBoolean() ?: true
